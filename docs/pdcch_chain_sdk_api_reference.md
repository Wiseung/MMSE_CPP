...
GPU 一站式 common-search / `DCI 1A` 入口，接受 `(n_tx_ports, tx_mode) == (1, 1)` 或
`(2, 2)`。2Tx 路径复用 TD pair 去映射，候选仍按 `first_cce * 72` 的连续 LLR 偏移处理。

约束：`20 MHz / FDD / normal CP / regular control subframe`，仅 native GPU tail-biting
...
- `run_pdcch_gpu_common_search_decode(...)` 接受 `1Tx` 或 `2Tx TD`，但仅覆盖 common-search / `DCI 1A`

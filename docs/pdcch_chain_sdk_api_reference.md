...
- `mmse::pdcch::run_pdcch_gpu_common_search_decode`
- `mmse::pdcch::run_pdcch_gpu_common_search_decode_batch`
...
1. 上游构造 `PdcchMmseInput`，固定 `n_tx_ports=4`、`n_rx_ant=1`、`tx_mode=2`
2. TD CE/MMSE 填充 `PdcchTd4MmseOutputView` 和 `PdcchTd4MmseResult`
3. `make_backend_pdcch_td4_equalized_indication(...)` 构造 owning DTO
4. `normalize_pdcch_td4_cce_order(...)` 归一化后复用标准 CCE/解码链
...
mmse::MmseEqualizerCpuContext::run_pdcch_td4(
    const PdcchMmseInput& in,
...
mmse::MmseEqualizerGpuContext::run_pdcch_td(
    ...
    PdcchTdMmseOutputView& out,
    PdcchTdMmseResult& meta)
mmse::MmseEqualizerGpuContext::run_pdcch_td4(
    const PdcchMmseInput& in,
    PdcchTd4MmseOutputView& out,
    PdcchTd4MmseResult& meta)
```
...
mmse::pdcch::make_backend_pdcch_td4_equalized_indication(...)
...
| `n_tx_ports`           | `uint8_t`                   | 本次请求的 CRS 发射端口数 | ports              | `1`、`2` 或 `4`                |
| `tx_mode`              | `uint8_t`                   | LTE 发射模式标签          | none               | 1Tx 为 `1`；TD2/Td4 为 `2`     |
| `control_symbol_count` | `uint8_t`                   | LTE 控制区大小            | OFDM symbols       | 6 RB 为 `1..4`，其它为 `1..3`  |
| `n_prb`                | `uint16_t`                  | 位图中启用的 PRB 数       | PRBs               | `6/15/25/50/75/100`            |
...
| `cell_id`                    | `uint16_t`                  | LTE 物理小区 ID            | none               | `0..503`                       |
| `n_tx_ports`                 | `uint8_t`                   | 发射端口数                 | ports              | `1`、`2` 或 `4`                |
| `tx_mode`                    | `uint8_t`                   | 发射模式标签               | none               | 1Tx 为 `1`；TD2/Td4 为 `2`     |
| `control_symbol_count`       | `uint8_t`                   | LTE 控制区大小             | OFDM symbols       | 6 RB 为 `1..4`，其它为 `1..3`  |
| `n_prb`                      | `uint16_t`                  | 激活 PRB 数                | PRBs               | `6/15/25/50/75/100`            |
| `prb_bitmap`                 | `array<uint16_t,7>`         | 激活 PRB 位图              | bitmap             | 必须恰好有 `n_prb` 个 bit 置位 |
| `control_subframe`           | `LteControlSubframeContext` | 共享 LTE 控制子帧上下文    | context            | helper 定义范围                |
...
### 2.7a `PdcchTdMmseOutputView / PdcchTdMmseResult`

TD2 output view 包含 `x_hat_re/x_hat_im/sinr`、`re_grid_indices0/1` 和
`capacity_symbols`。result 使用 `n_symbols` 与 `n_source_re` 标记有效前缀；两者相等且为
`2` 的倍数。每两个连续槽位重复同一对 source RE。

### 2.7b `PdcchTd4MmseOutputView / PdcchTd4MmseResult / BackendPdcchTd4EqualizedIndication`

Td4 output view 由调用方持有，包含 `x_hat_re/x_hat_im/sinr`、四个
`re_grid_indices0..3` 指针和 `capacity_symbols`。result 以 `n_symbols`、`n_source_re` 标记
有效前缀，并返回网格、端口、控制区、QPSK、噪声和链路元数据。owning backend DTO 深拷贝
同一有效前缀。

不变量：

- `n_symbols == n_source_re` 且为 `4` 的倍数
- 七个 owning 向量，即三个数值向量和四个索引向量，长度都等于 `n_symbols`
- 每个 `q = 0,4,8,...` 的槽位 `q..q+3` 重复同一组 `re_grid_indices0..3`
- 仅 `4Tx x 1Rx / 1 layer / tx_mode=2 / QPSK`
...
| `BackendPdcchTd4EqualizedIndication`  | 表示 `4Tx x 1Rx` Td4 owning 软符号输出     | 使用 `re_grid_indices0..3` 表达四源 RE 块                    |
...
| `n_subcarriers` | 网格宽度               | subcarriers    | CPU 为 `12*n_prb`；GPU 为 `1200` |
...
### `normalize_pdcch_td4_cce_order(td_backend, cce_ordered_backend)`

校验 `4Tx x 1Rx / 1 layer / tx_mode=2 / QPSK`，要求长度为 `4` 的倍数，且每四个连续槽位
重复同一组 `re_grid_indices0..3`。成功后复制软符号和 SINR，并依次将四个 source RE 写入
标准 `BackendPdcchEqualizedIndication::re_grid_indices`；输入非法时清空输出并返回
`kInvalidArgument`。
...
`run_pdcch(...)`、`run_pdcch_td(...)` 或 `run_pdcch_td4(...)`，对 TD 输出调用对应
normalize helper，随后执行 UE-specific helper 链路。同一 dispatcher 也由 CPU common-search、
固定 SI-RNTI 和 SI-RNTI geometry 路径复用。

约束：当前只支持 `FDD / normal CP / DCI 1A`；CPU 固定几何路径支持
`6/15/25/50/75/100 RB`，并接受 1Tx、2Tx TD 或 `4Tx x 1Rx` Td4。
...
### `run_pdcch_gpu_common_search_decode(...)`

GPU 一站式 common-search / `DCI 1A` 入口，接受 `(n_tx_ports, tx_mode) == (1, 1)`、
`(2, 2)` 或 `(4, 2)`；4Tx 额外要求 `n_rx_ant == 1`。TD 路径复用对应去映射，候选仍按
`first_cce * 72` 的连续 LLR 偏移处理。

约束：`100 RB / FDD / normal CP / regular control subframe`，仅 native GPU tail-biting
...
### `decode_re_grid_index(grid_index, n_prb)`
...
公式，其中 `n_subcarriers = 12 * n_prb`：

- `symbol = grid_index / n_subcarriers`
- `subcarrier = grid_index % n_subcarriers`
- `prb = subcarrier / 12`
...
- 仅 LTE
- CPU 固定几何入口支持 `6/15/25/50/75/100 RB`；GPU common-search 固定 `100 RB`
- 仅 normal CP
- `n_rx_ant == 1 or 2`
- `n_symbols == 14`
- CPU `n_subcarriers == 12 * n_prb`；GPU common-search `n_subcarriers == 1200`
- 仅 PDCCH
- `control_symbol_count in [1, 4]`（仅 6 RB 允许 `4`；其它带宽为 `1..3`）
- 对 PDCCH 而言 `mod_order == 2`
- `n_layers == 1`
- `tx_mode == 1 or 2`
- `run_pdcch_cpu_common_search_decode(...)`、`run_pdcch_cpu_si_rnti_search(...)` 与 `run_pdcch_cpu_ue_specific_search(...)` 接受 1Tx、2Tx TD 或 `4Tx x 1Rx` Td4
- `run_pdcch_gpu_common_search_decode(...)` 接受 1Tx、2Tx TD 或 `4Tx x 1Rx` Td4，但仅覆盖 common-search / `DCI 1A`
- 正式 blind-decode helper 覆盖 common-search 与 UE-specific `DCI 1A`
- `run_pdcch_cpu_si_rnti_geometry_search(...)` 支持 `6/15/25/50/75/100 RB / FDD / normal CP`，并接受 1Tx、2Tx TD 或 `4Tx x 1Rx` Td4

重要的不支持项：

- 传统 `run_pdcch(...)` 会拒绝 `2 Tx port` LTE PDCCH，因为其冻结契约是 per-RE
- `2 Tx port` 发射分集只通过新增的 `run_pdcch_td(...)` 支持
- `4 Tx port` 发射分集只通过 `run_pdcch_td4(...)` 支持，并固定为 `1Rx / 1 layer / tx_mode=2 / QPSK`
...
调用 `run_pdcch_td4(...)` 时使用同一容量规则，但有效长度必须为完整四元组。无法精确计算
控制区有效 RE 数时，可保守分配 `4 * 1200` 个槽位；只消费 `meta.n_symbols` 有效前缀。

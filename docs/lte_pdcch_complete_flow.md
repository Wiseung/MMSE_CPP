...
- 1Tx、2Tx TD 与 `4Tx x 1Rx` TD 的 GPU common-search / `DCI 1A` 一站式链路
...
- `run_pdcch_td4(...)` 对应 `4Tx x 1Rx` 的 transmit-diversity 专用输出面
...
`run_pdcch(...)`、`run_pdcch_td(...)` 或 `run_pdcch_td4(...)` 之前，已经把 `control_symbol_count`
...
1. 构造 `PdcchMmseInput`
2. 调用 `run_pdcch(...)`、`run_pdcch_td(...)` 或 `run_pdcch_td4(...)`
...
`4Tx x 1Rx` 使用 `run_pdcch_td4(...)`，并固定为单层、`tx_mode == 2`、QPSK。每四个
source RE 分成两个 Alamouti 对，恢复四个连续软符号；`re_grid_indices0..3` 在这四个槽位中
重复记录同一组 source RE。`normalize_pdcch_td4_cce_order(...)` 校验该四元组契约并把索引
重建为标准连续 CCE 顺序，软符号和 SINR 本身不重排。
...
5. `1Tx` 走 `run_pdcch(...)`，`2Tx TD` 走 `run_pdcch_td(...)`，`4Tx x 1Rx` 走 `run_pdcch_td4(...)`
...
11. 若需要 GPU common-search，可调用 `run_pdcch_gpu_common_search_decode(...)` 或 batch 入口；2Tx/Td4 自动保持连续 CCE 顺序，D2H 仅返回 compact hits
...
| `4Tx x 1Rx PDCCH TD`          | `run_pdcch_td4(...)`                                    |
| Td4 CCE 顺序归一化            | `normalize_pdcch_td4_cce_order(...)`                    |
...
| 1Tx/2Tx/4Tx common-search GPU | `run_pdcch_gpu_common_search_decode(...)`               |
| 正式 CPU 盲检索链路           | 见下文                                                  |
...
- 它知道如何处理 `1Tx`、`2Tx TD` 和 `4Tx x 1Rx` Td4 三种 `PDCCH` 软符号输出
...
GPU common-search 的更窄边界是 `100 RB / FDD / normal CP / regular control subframe`、
native decoder、无 CIF/外部回调；4Tx 仅接受 1Rx。

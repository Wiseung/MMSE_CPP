...
**控制区裁剪 -> RE 提取 -> CRS 信道估计 -> MMSE -> 软符号/LLR 输出**

而不是完整 LTE 接收机的最终 `DCI` 解码器。
...
3. 下游译码段  
   做 `QPSK` 软比特、`REG/CCE` 重组、搜索空间盲检索、卷积译码、`CRC/RNTI` 校验，并输出 `DCI`。
...
当前仓库没有内建 `REG` 重组器。它的设计选择是把所需信息透传给下游：

- `re_grid_indices`
- 对 `2Tx` 场景则是 `re_grid_indices0 / re_grid_indices1`

也就是说，仓库不替下游强行固定 `REG` 顺序；下游应根据 `re_grid_indices` 自己恢复协议需要的 `REG` 组织。

### 4.9 `CCE` 重组

在 LTE 里，一个 `CCE` 由 `9 REG` 构成。完成 `REG` 级恢复后，下一层就是把它们拼成 `CCE`，为后续搜索空间盲检索做准备。

当前仓库同样不内建这一步。它只做两件支持性工作：

- 透传 `re_grid_indices`，让下游有足够信息恢复 `REG/CCE`
- 透传 `PdcchChainMetadata`，让调用方在需要时把 `candidate_id / first_cce / aggregation_level`
  这类候选元数据一并带到后续链路

这里要注意一个事实：

- `PdcchChainMetadata` 是“调用方定义并透传”的元数据
- 它不是仓库内部自动执行盲检索后生成的结果
...
- 仓库不会枚举 `CCE` 候选
- 不会决定 `first_cce`
- 不会输出命中的 `aggregation_level`

如果系统已经在仓库外有自己的候选管理器，那么可以把这些字段通过
`PdcchChainMetadata` 一路透传。
...
7. 下游自己完成 `REG/CCE` 重组、盲检索、速率恢复、卷积译码、`CRC/RNTI` 校验和 `DCI` 输出
...
但从 `REG/CCE` 开始，到真正输出“哪条 `DCI` 命中、对应哪个 `RNTI`、内容是什么”为止，当前仍然需要一个仓库外的完整 `PDCCH decoder` 来接上。

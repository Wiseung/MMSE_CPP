...
这意味着本项目的核心职责边界是 equalized RE 输出，并额外提供文档化的 PDCCH `DCI 1A`
解码链。它不直接负责：

- PBCH 卷积译码 / MIB 恢复
- 非 `DCI 1A` 的通用 PDCCH 盲检索 / DCI 译码
...
- `4Tx` 当前只用于 `PBCH / PCFICH / PDCCH` 的 `4Tx x 1Rx` Td4 控制信道路径；
  不提供 4Tx PDSCH
...
对 PBCH/PCFICH/PDCCH Td4，布局按四个 source RE 形成完整四元组。每个四元组拆成两个
Alamouti 对，恢复四个连续 QPSK 软符号；`re_grid_indices0..3` 在四个输出槽位中重复记录
同一组 source RE。PDCCH 可在该输出上调用 `normalize_pdcch_td4_cce_order(...)`，重建标准
CCE source-RE 顺序后复用既有解码链。
...
当前 CPU 和 GPU 路径都遵循同一算法。CRS 时域位置按端口区分：port 0/1 使用
`{0,4,7,11}`，port 2/3 使用 `{1,8}`。
...
- port 0/1 使用 `kCrsSymbols = {0, 4, 7, 11}`
- port 2/3 使用 `kCrsSymbols = {1, 8}`
...
## 9. 2Tx 与 4Tx transmit-diversity 的处理
...
PBCH、PCFICH 和 PDCCH 还提供 `run_pbch_td4(...)`、`run_pcfich_td4(...)` 和
`run_pdcch_td4(...)`。这三条路径只接受 `4Tx x 1Rx / 1 layer / tx_mode=2 / QPSK`，利用
port 0/1 的 `{0,4,7,11}` CRS 与 port 2/3 的 `{1,8}` CRS 估计四条发送信道。每四个
source RE 按两个 Alamouti 对联合去分集并输出四个软符号。该能力不表示四层空间复用，
也不增加 4Tx PDSCH。
...
GPU 路径维护 `kPinnedRingSlotCount = 16` 的 host pinned ring buffer。

...
- GPU common-search / `DCI 1A` 一站式与 batch 链路

当前仓库不负责：

- 非 `DCI 1A` 的通用 `DCI` 解析 / 翻译
- GPU UE-specific、SI-RNTI geometry search、非 `DCI 1A` 格式、CIF 或外部 decoder callback
...
1. `run_pdcch(...)`、`run_pdcch_td(...)` 或 `run_pdcch_td4(...)` 输出 `x_hat + sinr + re_grid_indices`
...
- `n_rx_ant in {1, 2}`
- `n_symbols == 14`
- PBCH、PCFICH、PDSCH 和 GPU PDCCH 为 `n_subcarriers == 1200`
- CPU PDCCH 为 `n_subcarriers == 12 * n_prb`，支持 `6/15/25/50/75/100 RB`

所有入口都处理一个 normal-CP LTE `1 ms` 子帧；除 CPU PDCCH 可变带宽路径外，当前网格
固定为 20 MHz。

按 20 MHz planar `float` 布局计算，单次调用的输入网格数据量是：
...
软符号共享同一对来源 RE。
对 `4Tx x 1Rx` Td4，输出为 `re_grid_indices0..3`：每四个连续软符号共享并重复同一组
四个 source RE。PDCCH 可归一化为标准 CCE 顺序；PBCH/PCFICH 只提供 caller-owned raw output。
...
| `run_pbch_td4(...)`   | `PbchMmseInput`                         | `PbchTd4MmseOutputView + PbchTd4MmseResult`     | 固定 `240` 个 source `RE / soft symbols`                                                     |
| `run_pcfich_td4(...)` | `PcfichMmseInput`                       | `PcfichTd4MmseOutputView + PcfichTd4MmseResult` | 固定 `16` 个 source `RE / soft symbols`                                                      |
| `run_pdcch_td4(...)`  | `PdcchMmseInput`                        | `PdcchTd4MmseOutputView + PdcchTd4MmseResult`   | 随控制区变化，长度为完整四元组并做 CCE 对齐                                                  |
...
这些 CPU 入口按 `n_tx_ports` 分派 1Tx、2Tx TD 或 `4Tx x 1Rx` Td4；TD 输出先归一化为
标准 CCE 顺序。GPU common-search 同样覆盖这三种模式，但固定为
`100 RB / FDD / normal CP / regular control subframe`、native decoder、无 CIF/外部回调。
...
| ---------------------------------- | ---------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `PBCH` / `run_pbch(...)`           | `1`                    | 按 LTE PBCH 的 `10 ms` 周期计一次；wrapper 透传调用方提供的 `sfn_subframe`。                                                                              |
| `PCFICH` / `run_pcfich(...)`       | `10`                   | 每个 downlink 子帧最多一次。若按完整 `10` 个 LTE 子帧都需处理，则上界是 `10`。                                                                            |
| `PDCCH` / `run_pdcch(...)`         | `10`                   | 每个 downlink 子帧最多一次。1Tx、2Tx TD 与 4Tx Td4 是互斥模式，不会在同一条真实链路上重复计数。                                                           |
| `PDCCH TD` / `run_pdcch_td(...)`   | `10`                   | 仅当系统实际使用 `2 Tx port` PDCCH 发射分集时成立。                                                                                                       |
| `PDCCH Td4` / `run_pdcch_td4(...)` | `10`                   | 仅当系统实际使用 `4Tx x 1Rx` PDCCH 发射分集时成立。                                                                                                       |
...
1. 当前仓库负责 LTE `PBCH / PCFICH / PDCCH / PDSCH` 的 **RE 提取 + CRS 信道估计 + MMSE 均衡**；它不负责 MIB/TB 或非 `DCI 1A` 的通用最终译码。

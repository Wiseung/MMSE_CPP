...
| `PBCH` equalized `RE` 输出                      | 已支持     | `run_pbch(...)`；`4Tx x 1Rx` 使用 `run_pbch_td4(...)`                         |
| `PCFICH` equalized `RE` 输出                    | 已支持     | `run_pcfich(...)`；`4Tx x 1Rx` 使用 `run_pcfich_td4(...)`                     |
| `PDCCH 1Tx` equalized `RE` 输出                 | 已支持     | `run_pdcch(...)`                                                              |
| `PDCCH 2Tx` transmit-diversity 去映射输出       | 已支持     | `run_pdcch_td(...)`                                                           |
| `PDCCH` 4Tx x 1Rx transmit-diversity 去映射输出 | 已支持     | `run_pdcch_td4(...)`                                                          |
| `PDSCH 2Tx` transmit-diversity 去映射输出       | 已支持     | `run_pdsch_td(...)`                                                           |
...
| `PDCCH common-search + DCI 1A` GPU 解码         | 已支持     | `run_pdcch_gpu_common_search_decode(...)`，支持 1Tx、2Tx TD 与 `4Tx x 1Rx` TD |
...
- `run_pdcch_gpu_common_search_decode(...)` / `run_pdcch_gpu_common_search_decode_batch(...)`
...
- GPU common-search 自动复用 2Tx TD 或 `4Tx x 1Rx` TD 去映射，并保持标准连续 CCE 顺序；D2H 只返回 compact candidate hits，不回传完整 equalized grid 或 LLR；入口固定使用 native decoder 且拒绝 CIF/外部回调
- 仓库仍不支持 `PCFICH -> CFI` 硬判决、非 `DCI 1A` 的通用最终 `DCI` 解码；GPU PDCCH 仍固定为 `100 RB`，且不支持 UE-specific、SI-RNTI geometry search 或外部 decoder callback
...
| 统一 LTE SDK 入口                   | `mmse/lte_chain_sdk.h`   | `run_pbch / run_pcfich / run_pdcch` 及对应 `run_*_td4`                                        |
...
| `PDCCH 4Tx x 1Rx` TD 链路           | `mmse/pdcch_chain_sdk.h` | `run_pdcch_td4(...)` / `normalize_pdcch_td4_cce_order(...)`                                   |
...
1. 按 `n_tx_ports` 分派 `run_pdcch(...)`、`run_pdcch_td(...)` 或 `run_pdcch_td4(...)`，
   并将 TD 输出归一化为标准 CCE 顺序

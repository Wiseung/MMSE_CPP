...
覆盖 `100 RB / FDD / normal CP / regular control subframe` 的 1Tx 与 2Tx TD common-search。
它仍然**不是**完整 LTE
...
| `PDCCH common-search + DCI 1A` GPU 解码   | 已支持     | `run_pdcch_gpu_common_search_decode(...)`，支持 1Tx 与 2Tx TD             |
...
- GPU common-search 自动复用 2Tx TD 去映射，并保持标准连续 CCE 顺序；D2H 只返回 compact candidate hits，不回传完整 equalized grid 或 LLR
